/**
 * Reset script — drops all tables then re-creates them via the migration DDL.
 * WARNING: destroys all data. Development use only.
 * Run with: pnpm --filter @grimoire/server db:reset
 */
...
// Re-create by inlining the same DDL used in migrate.ts
const statements = [
